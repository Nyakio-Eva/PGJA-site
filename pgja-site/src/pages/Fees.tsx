...
export default function Fees() {
  return (
    <section className="bg-[#dfecfaff] text-gray-900">
      ...
      <div className="w-full overflow-hidden h-[60vh] relative lg:bg-fixed bg-scroll">
        <img
          src="/feespage.webp"
          alt="Fees Banner"
      ...
      <div className="md:max-w-2xl mx-auto px-2 py-12 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {feesData.map((fee, index) => (
          <Link
            key={index}
            to={fee.url}
            className="block bg-[#de9642] hover:bg-yellow-700 transition rounded-md p-2 shadow-md text-center font-bold text-lg text-white font-serif hover:shadow-lg"
          >
            {fee.title}
          </Link>
        ))}
